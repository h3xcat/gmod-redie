--[[
	...
	https://github.com/Mr-Gash/GMod-Deathrun/blob/master/deathrun/gamemode/cl_scoreboard.lua

	Following code is slightly modified to show additional team (Redies)
]]
...
local function Refresh()
	...
	-- REDIES
	local redies = team.GetPlayers(TEAM_REDIE)
	local redies_alive = false
	for k, v in pairs( redies ) do
		if v:IsRedie() then redies_alive = true break end
	end
	if redies_alive then
		CreateEmpty( 10 )
		CreateTeamThing( "Redies", Color( 160, 160, 160, 233 ) )

		for k, v in pairs( redies ) do
			if not v:IsRedie() then pool[#pool+1] = v continue end
			CreatePlayer(v)
		end
	else
		for k, v in pairs( redies ) do
